...
public class CreateUpdateEmployeeCommand implements ICommand {
    ...
    private Employee getEmployee(HttpServletRequest req) {
        try {
            ...
            employee.setId(GetInt.getInt(req.getParameter("id")));
            employee.setFirstName(req.getParameter("firstName"));
            employee.setLastName(req.getParameter("lastName"));
            employee.setEmail(req.getParameter("email"));
            employee.setSalary(req.getParameter("salary") == "" ? 0 : GetInt.getInt(req.getParameter("salary")));
            employee.setHireDate(req.getParameter("hireDate") == "" ? null : new SimpleDateFormat("yyyy-MM-dd").parse(req.getParameter("hireDate")));
            employee.setIdDepartment(req.getParameter("iddepartment") == "" ? 0 : GetInt.getInt(req.getParameter("iddepartment")));
            return employee;
